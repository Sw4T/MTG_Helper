<?php
    ...
    function getSQLStringCardValues($card, $extensionCode)
    {
        ...
        if (isset($card->text)) { // On fixe les erreurs d'encodage
            $fixedText = str_replace("'", "\'", $card->text);
            $fixedText = str_replace("—", "-", $fixedText);
        }

        foreach ($card->colorIdentity as $color) {
            $fixedColorsString .= $color . '/';
        }
        $fixedColorsString = substr($fixedColorsString, 0, -1);
        
        // On retourne le résultat de notre super traitement qui respecte la structure de la table cartes 
        return "('{$fixedName}', '{$extensionCode}', '{$fixedType}', '{$fixedColorsString}', {$fixedIsBaseTerrain}, '{$card->rarity}', {$fixedConvertedManaCost},
    ...
    {
        $sqlInsertCard = 'INSERT INTO `cartes`(`nom`, `codeExtension`, `type`, `couleur`, `estTerrainBase`, `rarete`, `coutConvertiMana`, `coutManaTexte`, `forceCreature`, `enduranceCreature`, `texte`, `urlImage`) VALUES ';
